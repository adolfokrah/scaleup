...
export const CallToActionBlock: React.FC<CTABlockProps> = ({ links, richText }) => {
  return (
    <div className="bg-gradient-to-t from-primary/20 to-white mt-16">
      <div className="max-w-4xl flex flex-col md:flex-row gap-4 justify-between container md:items-center border-t py-20 border-t-gray-200/60">
        {/* Rich Text Content */}
        <motion.div
        ...
        {links && links.length > 0 && (
          <motion.div
            className="flex flex-col sm:flex-row gap-4 flex-2 justify-start md:justify-end"
            initial={{ opacity: 0, y: 50 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{
              duration: 0.6,
